...
def report_maker(tool, sampleId, case, genes, results):
	...
	full_report.loc[full_report.gene1_mark != case, 'gene1'] = "undisclosed"
	full_report.loc[full_report.gene2_mark != case, 'gene2'] = "undisclosed"

	twohit_report = final_report.loc[final_report["2-hit"] == 1]
	twohit_report.to_csv(f"./Results/{sampleId}_{tool}_2Hit_Report.csv", sep=',')
	if (full_report.shape[0]!=0):
		full_report.to_csv(f"./Results/Full_Reports/{sampleId}_{tool}_Full_Report.csv", sep=',')
	if (twohit_report.shape[0] == 0):
		os.remove(f"./Results/{sampleId}_{tool}_2Hit_Report.csv")
	if (full_report.shape[0] == 0):
		with open(f"./Results/Full_Reports/Nofusions.txt", 'a+') as file:
			file.seek(0)
			data = file.read(100)
			if len(data) > 0 :
        			file.write("\n")
...
for file in os.listdir('./Results/Full_Reports/'):
	if fnmatch.fnmatch(file, '*Arriba_*.csv'):
		arriba_maskreport=pd.read_csv(f"./Results/Full_Reports/{file}", sep=',')
		arriba_maskreport=arriba_maskreport.drop(columns=['breakpoint1', 'breakpoint2'])
		arriba_maskreport.to_csv(f"./Results/Full_Reports/{sampleId}_Arriba_Full_Report.csv", sep=',')
	elif fnmatch.fnmatch(file, '*ArribaD*.csv'):
		arriba_D_maskreport=pd.read_csv(f"./Results/Full_Reports/{file}", sep=',')
		arriba_D_maskreport=arriba_D_maskreport.drop(columns=['breakpoint1', 'breakpoint2'])
		arriba_D_maskreport.to_csv(f"./Results/Full_Reports/{sampleId}_ArribaDiscarded_Full_Report.csv", sep=',')
	elif fnmatch.fnmatch(file, '*StarFusion*.csv'):
		starfusion_maskreport=pd.read_csv(f"./Results/Full_Reports/{file}", sep=',')
		starfusion_maskreport.drop(columns=['Fusion_Name', 'Left_Breakpoint', 'Right_Breakpoint'])
		starfusion_maskreport.to_csv(f"./Results/Full_Reports/{sampleId}_StarFusion_Full_Report.csv", sep=',')